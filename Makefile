...
build_debug: debug_directories
	cd ${debug_build_dir} ; if test ! -e EXECUTABLES ; then mkdir EXECUTABLES; fi; $(CMAKE) -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Debug $(cmake_run_flags) -DCMAKE_C_FLAGS="$(CFLAGS) -g3 -O0" -DCMAKE_CXX_FLAGS="$(CXXFLAGS) -g3 -O0" .. $(CMAKE_FLAGS) ; $(MAKE) $(MAKE_FLAGS) && if test -e EXECUTABLES ; then cd EXECUTABLES; for file in * ; do mv -v $$file ../../$(debug_executable_dir)/$$FILE ; done ; cd ..; rmdir EXECUTABLES; fi
...
build_release: release_directories
	cd ${release_build_dir} ; if test ! -e EXECUTABLES ; then mkdir EXECUTABLES; fi; $(CMAKE) -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release $(cmake_run_flags) -DCMAKE_C_FLAGS="$(CFLAGS) -g0 -O3" -DCMAKE_CXX_FLAGS="$(CXXFLAGS) -g0 -O3" .. $(CMAKE_FLAGS) ; $(MAKE) $(MAKE_FLAGS) && if test -e EXECUTABLES ; then cd EXECUTABLES; for file in * ; do mv -v $$file ../../$(release_executable_dir)/$$FILE ; done ; cd ..; rmdir EXECUTABLES; fi
