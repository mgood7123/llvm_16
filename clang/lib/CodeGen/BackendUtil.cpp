...
static bool setCommandLineOpts(const CodeGenOptions &CodeGenOpts) {
  ...
  return !llvm::cl::ParseCommandLineOptions(BackendArgs.size() - 1,
                                    BackendArgs.data());
}
...
bool EmitAssemblyHelper::EmitAssembly(BackendAction Action,
  ...
  if (!setCommandLineOpts(CodeGenOpts))
    return false;
...
void clang::EmitBackendOutput(DiagnosticsEngine &Diags,
  ...
  if (!AsmHelper.EmitAssembly(Action, std::move(OS)))
    return;
