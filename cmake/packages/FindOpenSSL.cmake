...
function(_OpenSSL_add_dependencies libraries_var)
  if(_OpenSSL_has_dependency_zlib)
    list(APPEND ${libraries_var} ${ZLIB_LIBRARIES})
  endif()
...
function(_OpenSSL_target_add_dependencies target)
  if(_OpenSSL_has_dependencies)
    if(_OpenSSL_has_dependency_zlib)
      set_property( TARGET ${target} APPEND PROPERTY INTERFACE_LINK_LIBRARIES ZLIB )
    endif()
    if(_OpenSSL_has_dependency_threads)
...
find_path(OPENSSL_INCLUDE_DIR NAMES openssl/ssl.h
  PATHS ${CMAKE_CURRENT_BINARY_DIR}/BUILD_ROOT/ROOTFS/include
  NO_DEFAULT_PATH
...
if(OPENSSL_FOUND)
  if(NOT TARGET LLVM_STATIC_CRYPTO AND
      (EXISTS "${OPENSSL_CRYPTO_LIBRARY}" OR
        EXISTS "${LIB_EAY_LIBRARY_DEBUG}" OR
        EXISTS "${LIB_EAY_LIBRARY_RELEASE}")
      )
    add_library(LLVM_STATIC_CRYPTO UNKNOWN IMPORTED)
    set_target_properties(LLVM_STATIC_CRYPTO PROPERTIES
    ...
    set(OPENSSL_CRYPTO_TARGET LLVM_STATIC_CRYPTO)
    if(EXISTS "${LIB_EAY_LIBRARY_RELEASE}")
      set_property(TARGET LLVM_STATIC_CRYPTO APPEND PROPERTY
    ...
    _OpenSSL_target_add_dependencies(LLVM_STATIC_CRYPTO)
  endif()

  if(NOT TARGET LLVM_STATIC_SSL AND
      (EXISTS "${OPENSSL_SSL_LIBRARY}" OR
        EXISTS "${SSL_EAY_LIBRARY_DEBUG}" OR
        EXISTS "${SSL_EAY_LIBRARY_RELEASE}")
      )
    add_library(LLVM_STATIC_SSL UNKNOWN IMPORTED)
    set_target_properties(LLVM_STATIC_SSL PROPERTIES
      INTERFACE_INCLUDE_DIRECTORIES "${OPENSSL_INCLUDE_DIR}")
    ...
    set(OPENSSL_SSL_TARGET LLVM_STATIC_SSL)
    if(EXISTS "${SSL_EAY_LIBRARY_RELEASE}")
      set_property(TARGET LLVM_STATIC_SSL APPEND PROPERTY
        IMPORTED_CONFIGURATIONS RELEASE)
      set_target_properties(LLVM_STATIC_SSL PROPERTIES
        IMPORTED_LINK_INTERFACE_LANGUAGES_RELEASE "C"
    ...
    if(TARGET LLVM_STATIC_CRYPTO)
      set_target_properties(LLVM_STATIC_SSL PROPERTIES
        INTERFACE_LINK_LIBRARIES LLVM_STATIC_CRYPTO)
    endif()
    _OpenSSL_target_add_dependencies(LLVM_STATIC_SSL)
  endif()
...
message(STATUS "OpenSSL: include_dirs : ${OPENSSL_INCLUDE_DIR}")
