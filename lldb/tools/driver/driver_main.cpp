...
#ifdef LLDB_DRIVER_MAIN_LIBRARY_ENABLE
...
int lldb_main(int argc, char const *argv[]) {
  ...
  auto sigint_handler_old = signal(SIGINT, sigint_handler);
#if !defined(_WIN32)
  auto sigpipe_handler_old = signal(SIGPIPE, SIG_IGN);
  auto sigwinch_handler_old = signal(SIGWINCH, sigwinch_handler);
  auto sigstp_handler_old = signal(SIGTSTP, sigtstp_handler);
#endif
  ...
  // before SBDebugger::Terminate() is called.
  {
    ...
    if (error.Fail()) {
      ...
      llvm::outs() << "enter main loop\n";
      exit_code = driver.MainLoop();
      llvm::outs() << "leave main loop\n";
    }
  }
...
#if !defined(_WIN32)
  signal(SIGPIPE, sigpipe_handler_old);
  signal(SIGWINCH, sigwinch_handler_old);
  signal(SIGTSTP, sigstp_handler_old);
#endif

  return exit_code;
}

#endif // LLDB_DRIVER_MAIN_LIBRARY_ENABLE