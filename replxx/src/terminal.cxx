...
// If _DEBUG_LINUX_KEYBOARD is set, then ctrl-B puts us into a keyboard
// debugging mode
...
//
	...
	if (c == Replxx::KEY::control('B')) {	// ctrl-B, special debug mode, prints all keys hit,
														 // ctrl-C to get out
		printf(
				"\nEntering keyboard debugging mode (on ctrl-B), press ctrl-C to exit "
				"this mode\n");
		while (true) {
			char32_t key = read_unicode_character();

			key = EscapeSequenceProcessing::doDispatch(key);
			if ( is_control_code( key ) ) {
				key = Replxx::KEY::control( control_to_human( key ) );
			}
			char* friendlyTextPtr;
			char friendlyTextBuf[10];
			const char* prefixText = (key < 0x80) ? "" : "0x80+";
			if (key >= '!' && key <= '~') {	// printable
				friendlyTextBuf[0] = '\'';
			...
			} else if ((key & 0x04000000) != 0) {
				friendlyTextBuf[0] = 'E';
				friendlyTextBuf[1] = 'S';
				friendlyTextBuf[2] = 'C';
				friendlyTextBuf[3] = '+';
				char32_t key_ = ((key) & ~(0x04000000));
				if (key_ >= '!' && key_ <= '~') {	// printable
					friendlyTextBuf[4] = '\'';
					friendlyTextBuf[5] = key_;
					friendlyTextBuf[6] = '\'';
					friendlyTextBuf[7] = 0;
				} else if (key_ == ' ') {
					friendlyTextBuf[4] = 's';
